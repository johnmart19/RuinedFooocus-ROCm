...
def build_dynamic_prompt(insanitylevel = 5, forcesubject = "all", artists = "all", imagetype = "all", onlyartists = False, antivalues = "", prefixprompt = "", suffixprompt ="",promptcompounderlevel ="1", seperator = "comma", givensubject="",smartsubject = True,giventypeofimage="", imagemodechance = 20, gender = "all", subtypeobject="all", subtypehumanoid="all", subtypeconcept="all", advancedprompting=True, hardturnoffemojis=False, seed=-1, overrideoutfit="", prompt_g_and_l = False, base_model = "SD1.5", OBP_preset = "", prompt_enhancer = "none", subtypeanimal="all", subtypelocation="all", preset_prefix = "", preset_suffix = ""):
    ...
    if(OBP_preset == OBPresets.RANDOM_PRESET_OBP):
        ...
        subtypeobject = selected_opb_preset.get("chosensubjectsubtypeobject", "all")
        subtypehumanoid = selected_opb_preset.get("chosensubjectsubtypehumanoid", "all")
        subtypeconcept = selected_opb_preset.get("chosensubjectsubtypeconcept", "all")
        gender = selected_opb_preset.get("chosengender", "all")
        imagetype = selected_opb_preset.get("imagetype", "all")
        imagemodechance = selected_opb_preset.get("imagemodechance", 20)
        givensubject = selected_opb_preset.get("givensubject", "")
        smartsubject = selected_opb_preset.get("smartsubject", True)
        overrideoutfit = selected_opb_preset.get("givenoutfit", "")
        prefixprompt = selected_opb_preset.get("prefixprompt", "")
        suffixprompt = selected_opb_preset.get("suffixprompt", "")
...
def cleanup(completeprompt, advancedprompting, insanitylevel = 5):
    ...
    while "  " in completeprompt:
        completeprompt = re.sub(r'  ', ' ', completeprompt)
    completeprompt = re.sub(r'a The', 'The', completeprompt)
    completeprompt = re.sub(r'the the', 'the', completeprompt)
    completeprompt = re.sub(r', ,', ',', completeprompt)
    completeprompt = re.sub(r',,', ',', completeprompt)

    completeprompt = re.sub(r', of a', ' of a', completeprompt)
    completeprompt = re.sub(r'of a,', 'of a', completeprompt)
    completeprompt = re.sub(r'of a of a', 'of a', completeprompt)
    completeprompt = re.sub(r' a a ', ' a ', completeprompt)

    # a / an
    completeprompt = re.sub(r' a a', ' an a', completeprompt)
    completeprompt = re.sub(r' a e', ' an e', completeprompt)
    completeprompt = re.sub(r' a i', ' an i', completeprompt)
    completeprompt = re.sub(r' a u', ' an u', completeprompt)
    completeprompt = re.sub(r' a o', ' an o', completeprompt)


    completeprompt = re.sub(r'art art', 'art', completeprompt)
    completeprompt = re.sub(r'Art art', 'art', completeprompt)
    completeprompt = re.sub(r'lighting lighting', 'lighting', completeprompt)
    completeprompt = re.sub(r'Lighting lighting', 'lighting', completeprompt)
    completeprompt = re.sub(r'light lighting', 'light', completeprompt)
    completeprompt = re.sub(r'-artiststyle- art,', '', completeprompt)
    completeprompt = re.sub(r'-artiststyle- art', '', completeprompt)
    completeprompt = re.sub(r'-artiststyle-', '', completeprompt)
    completeprompt = re.sub(r'-artistmedium-', '', completeprompt)
    completeprompt = re.sub(r'-artistdescription-', '', completeprompt)
    completeprompt = re.sub(r'- art ', '', completeprompt)

    completeprompt = re.sub(r'anime anime', 'anime', completeprompt)
    completeprompt = re.sub(r'anime, anime', 'anime', completeprompt)

    completeprompt = re.sub(r'shot shot', 'shot', completeprompt)
    

    completeprompt = re.sub(r'a his', 'his', completeprompt)
    completeprompt = re.sub(r'a her', 'her', completeprompt)
    completeprompt = re.sub(r'they is', 'they are', completeprompt)
    completeprompt = re.sub(r'they has', 'they have', completeprompt)

    # some space tricks
    completeprompt = re.sub(r'- shaped', '-shaped', completeprompt)
    completeprompt = re.sub(r'echa- ', 'echa-', completeprompt)
    completeprompt = re.sub(r'style -', 'style-', completeprompt)
    completeprompt = re.sub(r', as a', ' as a', completeprompt)


    #small fix for multisubject thing
    completeprompt = re.sub(r'a 2', '2', completeprompt)
    completeprompt = re.sub(r'a 3', '3', completeprompt)
    completeprompt = re.sub(r'a 4', '4', completeprompt)
    completeprompt = re.sub(r'a 5', '5', completeprompt)


    # clean up some hacky multiples with adding a s to the end
    completeprompt = re.sub(r'fs ', 'ves ', completeprompt)
    completeprompt = re.sub(r'fs,', 'ves,', completeprompt)
    completeprompt = re.sub(r'sss ', 'ss ', completeprompt)
    completeprompt = re.sub(r'sss,', 'ss,', completeprompt)
    ...
    completeprompt = re.sub(r'-sameothersubject-', 'it', completeprompt)
    completeprompt = re.sub(r'-samehumansubject-', 'the person', completeprompt)
